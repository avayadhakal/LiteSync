...
- **Docker Config Enhancements:** Added `.dockerignore` to secure image builds and implemented `LITESYNC_PORT`, `LITESYNC_ALLOWED_ORIGINS`, and `LITESYNC_MAX_UPLOAD_SIZE_MB` environment variables to allow minimal `config.toml` files when using Docker Compose.
